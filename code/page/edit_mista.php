            ...
            <form action="#" method="post" enctype="multipart/form-data">
                ...
                <table>
                    <tr>
                        ...
                        <td><input type="number" name="obec"></td>
                    </tr>
                    <tr>
                        <td><label for="odbernmisto">Odběrné místo</label>
                        <td><input type="number" name="odbermisto"></td>
                    </tr>
                    <tr>
                        <td><label for="ulice">Ulice</label>
                        <td><input type="number" name="ulice"></td>
                    </tr>
                    <tr>
                        <td><label for="cp_ce">CP_CE</label>
                        <td><input type="text" name="cp_ce"></td>
                    </tr>
                    <tr>
                        <td><label for="cislodomu">Číslo popisné</label>
                        <td><input type="text" name="cislodomu"></td>
                    </tr>
                    <tr>
                    ...
                    <!-- nove prirazeny vodomer nesmi mit zadne odberne misto - kdyz uz ma, chyba -->
                    <?php
                    } else { ?>
                    ...
                    <table>
                        <tr>
                            <td><label for="obec">Obec</label></td>
                            <td><input type="number" name="obec" value="<?php echo $obec; ?>"></td>
                        </tr>
                        <tr>
                            <td><label for="odbernmisto">Odběrné místo</label></td>
                            <td><input type="number" name="odbermisto" value="<?php echo $odbermisto; ?>"></td>
                        </tr>
                        <tr>
                            <td><label for="ulice">Ulice</label></td>
                            <td><input type="number" name="ulice" value="<?php echo $ulice; ?>"></td>
                        </tr>
                        <tr>
                            <td><label for="cp_ce">CP_CE</label></td>
                            <td><input type="text" name="cp_ce" value="<?php echo $cp_ce; ?>"></td>
                        </tr>
                        <tr>
                            <td><label for="cislodomu">Číslo popisné</label></td>
                            <td><input type="text" name="cislodomu" value="<?php echo $cislodomu; ?>"></td>
                        </tr>
                        <tr>
                            <td><label for="parcela">Parcela</label></td>
                            <td><input type="text" name="parcela" value="<?php echo $parcela; ?>"></td>
                        </tr>
                        <?php }
                        if (!isset($_GET['id_upravit'])) {
               ...
               <option  value="' . $radek["ID"] . '">' . $radek["CISLO_VODOMERU"] . '</option>';
                        ...
                        <td><label for="select_sazby">Sazba</label></td>';
                            echo '<td><select name="select_sazby">';
                            while ($radek = $q->fetch(PDO::FETCH_ASSOC)) {
                                echo '            
...
echo'<tr>
        ...
        /* if (isset($_POST["upravit"])) {
             try {
                 $sql = 'select IDCISELPOD, ID_VODOMER, ODBERMISTO, TYP_SAZBY,OBEC, ULICE, CP_CE,CISLODOMU,PARCELA from odbernamista';
                 $q = $pdo->prepare($sql);
                 $q->execute();
             } catch (PDOException $e) {
                 echo "Error: " . $e->getMessage();
             }
             $datatable = new DataTable($q);
             $datatable->addColumn("IDCISELPOD", "IDCISELPOD");
             $datatable->addColumn("ID_VODOMER", "ID_VODOMER");
             $datatable->addColumn("ODBERMISTO", "Odběrné místo");
             $datatable->addColumn("TYP_SAZBY", "Typ sazby");
             $datatable->addColumn("OBEC", "Obec");
             $datatable->addColumn("ULICE", "Ulice");
             $datatable->addColumn("CP_CE", "Cp/Ce");
             $datatable->addColumn("CISLODOMU", "Číslo domu");
             $datatable->addColumn("PARCELA", "Parcela");
             $datatable->render();
         }*/
        ?>
