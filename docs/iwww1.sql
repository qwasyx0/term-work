...
--
-- Počítač: 127.0.0.1
-- Vytvořeno: Čtv 17. led 2019, 09:57
...
(2006, 1006, 6, 3006, 4006, 4, '2018-01-01', '2016-12-31', '780.00', '759.00', '155.00', '178.00'),
...
(2016, 1006, 6, 3006, 4006, 4, '2019-01-17', '2019-01-16', '802.00', '801.00', '31.00', '35.65');
...
(4014, 1002, 3001, '2019-01-16', 2, 'Prodej ');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `viewpohyby`  AS  select `p`.`DATUM_POHYBU` AS `DATUM_POHYBU`,`p`.`DRUH_POHYBU` AS `DRUH_POHYBU`,`p`.`POPIS_POHYBU` AS `POPIS_POHYBU`,`v`.`CISLO_VODOMERU` AS `CISLO_VODOMERU`,`v`.`DRUH_VODOMERU` AS `DRUH_VODOMERU`,`c`.`IDCISELPOD` AS `IDCISELPOD` from ((`vodomerypohyby` `p` left join `vodomery` `v` on((`v`.`ID` = `p`.`ID_VODOMERY`))) left join `ciselpod` `c` on((`c`.`IDCISELPOD` = `v`.`IDCISELPOD`))) ;
